...
def retweetsByDate(retweetsDF,days,volumesDF):
    ...
    tweetsDF = pd.read_csv('amc-tweets-emotion.csv',dtype={'id': str, 'author.id':str}) ##
    ...
    for day in days:
        ...
        nx.readwrite.write_edgelist(digraphTCC,'AMC'+str(flag)+'edgelist.csv',delimiter=',',data=['weight'])
        flag += 1
...
if __name__ == '__main__':
    ...
    retweetsDF = pd.read_csv('amc-rts-01-07.csv',dtype=str) ##
    
    volumesDF = pd.read_csv('AMC.csv') ##
