...
def retweetsByDate(retweetsDF,days,volumesDF):
    ##replace 'xyz' with the symbol of the stock in question, the jsonl files produced by hydrating the tweetIDs can be used instead
    tweetsDF = pd.read_csv('xyz-tweets.csv',dtype={'id': str, 'author.id':str}) ##
    ...
    for day in days:
        ...
        #Save each edgelist along the way
        nx.readwrite.write_edgelist(digraphTCC,'GME'+str(flag)+'edgelist.csv',delimiter=',',data=['weight'])
        flag += 1
        
        headers = ['date','size','edges','diameter','clustering','core_percent','anger','disgust','fear','joy','sadness','surprise','volume','daychange']

        centralities = [] #UN-comment any variable to be calculated. Be wary of the different runtimes for each graph metric (some of the graphs are quite large) -- Change headers accordingly
        '''
        #date,size,edges
        ...
        outCent = nx.algorithms.centrality.out_degree_centrality(todayG)
        centralities.append(sum(outCent.values()))
        closeCent = nx.algorithms.centrality.closeness_centrality(todayG)
        centralities.append(sum(closeCent.values()))

        #clustering,transitivity
        centralities.append(nx.algorithms.cluster.average_clustering(todayG,weight='weight'))
        centralities.append(nx.algorithms.cluster.transitivity(todayG))
...
if __name__ == '__main__':
    ...
    retweetsDF = pd.read_csv('xyz-rts.csv',dtype=str) ##
    
    volumesDF = pd.read_csv('XYZ.csv') ##
